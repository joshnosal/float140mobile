...
export default function CryptoChart(props){
  ...
  useEffect(()=>getData(), [props.period])
  ...
  const getData = () => {
    ...
    async.waterfall([
      ...
      (data, done)=>{
        let array = [], j=0
        if (props.period === '1hr' || props.period === '24hr') {
        ...
        } else if (props.period === '7d' || props.period === '30d' || props.period === '1yr' || props.period === '5yr') {
          let chunk = props.period === '7d' ? 2 : props.period === '30d' ? 8 : props.period === '1yr' ? 4 : 20
          for (let i=0; i<data.length; i++) {
            let max = i, curArrays = {
              lows: [],
              highs: [],
  ...
  const XAxis = () => {
    ...
    tickPaths.push({
      x1: margins.left - 1,
      x2: margins.left - 1,
      y1: props.height * windowWidth-margins.top,
      y2: props.height * windowWidth-margins.top + tickSize
    })
    tickTexts.push({
      text: labels[0],
      textAnchor: 'start',
      x: margins.left - 1,
      y: props.height * windowWidth-margins.top + tickSize + textSize,
    })
    if (props.period === '7d') {
      for (let i=1; i<6; i++) {
        tickPaths.push({
    ...
    return (
      <G>
        <Line 
          ...
          x2={props.width * windowWidth - margins.left + 1} 
          y1={props.height * windowWidth - margins.top } 
          y2={props.height * windowWidth - margins.top } 
    ...
    return (
      <G>
        <Line 
          ...
          x2={margins.left - 1} 
          y1={margins.top} 
          y2={props.height * windowWidth-margins.top + 1} 
          stroke={colors.primary} 
          strokeWidth={0.8}
        />
      </G>
  ...
  return (
    ...
    >
      ...
      <Svg height={props.height * windowWidth} width={props.width * windowWidth}>
        <XAxis/>
        <YAxisLeft/>
        <YAxisRight/>
